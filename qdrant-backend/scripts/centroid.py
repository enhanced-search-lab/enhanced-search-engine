...
from app.embeddings import (
    ...
    EMBED_DIM,
    build_query_embedding_from_seed_papers,  # <-- tek query embedding için
)
...
def main() -> None:
    ...
    # 4) TÜM abstract'lardan TEK bir query embedding üret
    print("Building single query embedding from all abstracts...\n")
    query_vec = build_query_embedding_from_seed_papers(
        abstracts=abstracts,
        shared_keywords=None,  # istersen ileride Gemini'den çıkardığın ortak keyword'leri buraya verebilirsin
    )

    if query_vec is None:
        print("Failed to build query embedding from seed abstracts.")
        return
    ...
    scored: List[Tuple[float, str, str]] = []  # (sim, openalex_id, title)
    ...
    for w in all_works_by_id.values():
        ...
        sim = float(np.dot(query_vec, v))  # cosine similarity (vektörler normalize)
