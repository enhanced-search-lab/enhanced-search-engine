...
from app.embeddings import (
    build_final_embedding_for_work,
    EMBED_DIM,
    build_query_embedding_from_seed_papers,  # tek query embedding
)
...
def abstract_to_openalex_query_heuristic(abstract: str, max_terms: int = 3) -> str:
    ...
    (Çıkış formatı: boşlukla ayrılmış kelimeler, phrase parsing kısmı bunu
     handle edecek.)
    """
    text = abstract.lower()
    ...
    # Boşlukla ayırıyoruz, phrase-parser gerekirse bunu .split ile bölecek.
...
def abstract_to_openalex_query_gemini(abstract: str, max_terms: int = 3) -> str:
    ...
    Eğer bir hata olursa, heuristik keyword extraction'a düşer.
    """
    try:
...
{abstract}
    ...
    # Çıkan cevabı direkt döndürüyoruz; parse işini main içinde yapacağız.
    return raw
...
def fetch_openalex_candidates(query: str, per_page: int = 30) -> List[dict]:
    """
    Call OpenAlex with a very short keyword/phrase query (search=...) and some filters.
...
def embed_query_from_abstract(abstract_text: str) -> np.ndarray:
    """
    (Şu an kullanılmıyor ama istersen tutabilirsin.)
...
def fetch_with_progressive_relaxation(tokens: List[str], per_page: int = 30, min_terms: int = 1) -> Tuple[List[dict], str]:
    """
    Verilen token/phrase listesinden:
    ...
    while len(current_tokens) >= min_terms:
        ...
        # Rastgele bir pozisyondan token/phrase sil
...
def main() -> None:
    ...
    for idx, abs_text in enumerate(abstracts, start=1):
        ...
        # Önce ; ile phrase'lere bölmeyi dene (Gemini protokolü)
        phrases = [p.strip() for p in raw_query.split(";") if p.strip()]

        # Eğer ; yoksa veya boşsa (heuristic veya bozuk çıktı) → eski split'e dön
        if not phrases:
            phrases = raw_query.split()

        # En fazla max_terms kadar phrase al
    ...
    for pos in range(num_positions):
        # Bu pozisyondaki phrase'leri tüm abstract'lardan topla
        ...
        # Case-insensitive duplicate temizliği
        seen = set()
    ...
    if user_keywords:
        ...
        works_kw, final_query_kw = fetch_with_progressive_relaxation(
            tokens=user_keywords,
            per_page=30,
            min_terms=1,
        )

        print(f"Final query used for user keywords: \"{final_query_kw}\"")
        print(f"Total works collected for user keywords: {len(works_kw)}\n")
        ...
        print(f"Total unique candidate works collected (with user keywords): {len(all_works_by_id)}\n")

    if not all_works_by_id:
        print("No candidates collected from OpenAlex. Try different abstracts or relax filters.")
        return

    # 4) TÜM abstract'lardan + user keywords'ten TEK bir query embedding üret
    print("Building single query embedding from all abstracts (and keywords if any)...\n")

    query_vec = build_query_embedding_from_seed_papers(
        abstracts=abstracts,
        shared_keywords=user_keywords if user_keywords else None,
    )

    if query_vec is None:
        print("Failed to build query embedding from seed abstracts / keywords.")
        return

    query_vec = np.asarray(query_vec, dtype="float32")
    query_vec /= (np.linalg.norm(query_vec) + 1e-8)

    # 5) Tüm candidate makaleleri TEK query embedding'e göre re-rank et
    scored: List[Tuple[float, str, str]] = []  # (sim, id, title)
    ...
    for w in all_works_by_id.values():
        ...
        sim = float(np.dot(query_vec, v))  # cosine similarity
